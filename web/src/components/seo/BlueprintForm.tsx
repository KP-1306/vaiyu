  ...
  return (
    ...
    >
      ...
      <section className="rounded-2xl border border-slate-800 bg-[#0F1320] p-4 space-y-3">
        <h3 className="text-sm font-semibold text-slate-100">{t('form.sectionConcept', 'Concept')}</h3>

        <Field label={t('form.pageTitleConcept', 'Page-title concept')} error={errors.TITLE_REQUIRED ? humanizeError('TITLE_REQUIRED', t) : errors.TITLE_TOO_LONG ? humanizeError('TITLE_TOO_LONG', t) : undefined}>
          <input
            type="text"
            value={draft.pageTitleConcept}
            onChange={(e) => setDraft((d) => ({ ...d, pageTitleConcept: e.target.value }))}
            placeholder={t('form.pageTitlePlaceholder', 'e.g. "Family stay in Mukteshwar"')}
            maxLength={160}
        ...
        <Field label={t('form.targetCategory', 'Target category')}>
          <select
          ...
          <p className="mt-1 text-[11px] text-slate-500">{t(`categoryHint.${draft.targetCategory}`, SEO_CATEGORY_HINT[draft.targetCategory])}</p>
        </Field>
      </section>

      {/* Live Policy Shield + Proof checklist */}
      <section className="rounded-2xl border border-slate-800 bg-[#0F1320] p-4 space-y-3">
        <h3 className="text-sm font-semibold text-slate-100">{t('shield.name', 'Policy Shield')}</h3>
        <PolicyShieldBanner risk={computedRisk} />

        <div>
          <p className="text-[10px] font-semibold uppercase tracking-wide text-slate-500 mb-1.5">
            {t('shield.checklistLabel', 'Proof checklist (advisory in v0)')}
          </p>
      ...
      {/* Guidance */}
      <section className="rounded-2xl border border-slate-800 bg-[#0F1320] p-4 space-y-3">
        <h3 className="text-sm font-semibold text-slate-100">{t('form.sectionGuidance', 'Guidance')}</h3>

        <Field label={t('form.whyItMatters', 'Why it matters (optional)')}>
          <textarea
            value={draft.whyItMatters}
            onChange={(e) => setDraft((d) => ({ ...d, whyItMatters: e.target.value }))}
            placeholder={t('form.whyItMattersPlaceholder', 'What kind of guest would land here, and what would convert them?')}
            rows={3}
            maxLength={2000}
        ...
        <Field label={t('form.hinglishGuidance', 'Hinglish guidance (optional)')}>
          <textarea
            value={draft.hinglishGuidance}
            onChange={(e) => setDraft((d) => ({ ...d, hinglishGuidance: e.target.value }))}
            placeholder={t('form.hinglishGuidancePlaceholder', 'e.g. "Asli photos chahiye, Char Dham route ka time honestly likhna."')}
            rows={2}
            maxLength={2000}
            className="w-full rounded-md border border-slate-700 bg-[#0B0E14] px-3 py-2 text-sm text-slate-100 placeholder:text-slate-600 focus:border-emerald-400 focus:outline-none"
          />
        ...
        <Field label={t('form.safeNextAction', 'Safe next action (optional)')}>
          <input
            type="text"
            value={draft.safeNextAction}
            onChange={(e) => setDraft((d) => ({ ...d, safeNextAction: e.target.value }))}
            placeholder={t('form.safeNextActionPlaceholder', 'e.g. "Upload 5 real photos and a local-attractions list"')}
            maxLength={1000}
            className="w-full rounded-md border border-slate-700 bg-[#0B0E14] px-3 py-1.5 text-sm text-slate-100 placeholder:text-slate-600 focus:border-emerald-400 focus:outline-none"
          />
        </Field>

        <Field label={t('form.connectedModule', 'Connected module suggestion (optional)')}>
          <select
          ...
          >
            {SEO_CONNECTED_MODULE_OPTIONS.map((o) => (
              <option key={o.value} value={o.value}>{t(`connectedModule.${o.value || 'none'}`, o.label)}</option>
            ))}
          </select>
        </Field>
      </section>
      ...
      {/* Notes */}
      <section className="rounded-2xl border border-slate-800 bg-[#0F1320] p-4 space-y-3">
        <h3 className="text-sm font-semibold text-slate-100">{t('form.sectionNotes', 'Notes')}</h3>
        <Field label={t('form.ownerNotes', 'Owner notes (visible to all hotel members)')}>
          <textarea
        ...
        <Field label={t('form.internalNotes', 'Internal notes (manager-level context)')}>
          <textarea
        ...
        <section className="rounded-2xl border border-amber-500/30 bg-amber-500/5 p-4 space-y-3">
          <div className="flex items-start gap-2">
            ...
            <div className="space-y-1">
              <p className="text-sm font-semibold text-amber-100">{t('override.title', 'Override the Policy Shield')}</p>
              <p className="text-[11px] text-amber-200/80">
                {t('override.body', "Use only when a human assertion overrides the deterministic flag (e.g. you've verified a fake local claim, or are parking this idea). A reason is required and recorded in audit.")}
              </p>
            </div>
          </div>
          <Field label={t('override.label', 'Override risk to')}>
            <select
              value={riskOverride ?? ''}
            ...
            >
              <option value="">{t('override.noOverride', '— No override (use computed: {{risk}}) —', { risk: t(`risk.${computedRisk}`, SEO_RISK_LABEL[computedRisk]) })}</option>
              {OVERRIDE_OPTIONS.map((r) => (
                <option key={r} value={r}>{t(`risk.${r}`, SEO_RISK_LABEL[r])}</option>
              ))}
            </select>
          </Field>
          {riskOverride && (
            <Field
              label={t('override.reasonLabel', 'Reason (required)')}
              error={errors.OVERRIDE_REASON_REQUIRED ? humanizeError('OVERRIDE_REASON_REQUIRED', t) : undefined}
            >
              <input
                type="text"
                value={overrideReason}
                onChange={(e) => setOverrideReason(e.target.value)}
                placeholder={t('override.reasonPlaceholder', 'e.g. "Verified false claim — landmark is actually 80km away."')}
                className="w-full rounded-md border border-slate-700 bg-[#0B0E14] px-3 py-1.5 text-sm text-slate-100 placeholder:text-slate-600 focus:border-emerald-400 focus:outline-none"
      ...
      <div className="flex items-center justify-end gap-2 pt-1">
        ...
        >
          {busy ? t('form.saving', 'Saving…') : submitLabel}
        </button>
      </div>
    </form>
  );

  ...
  return (
    <main className="max-w-6xl mx-auto p-4 space-y-5" aria-labelledby="guest-dash-title">
      ...
      <section className="grid md:grid-cols-3 gap-4">
        {/* Ad-hoc check-in only (no PMS required) */}
            ...
            <ul className="space-y-2">
              {reviews.data.map((rv) => (
                <li key={rv.id} className="text-sm">
                  <span className="font-semibold">{rv.hotel.name}</span> ·{" "}
                  {stars(rv.rating)} · {fmtDate(rv.created_at)}
                  {rv.title ? (
                    <>
                      {" "}
...
function demoStays(): Stay[] {
  return [
    ...
    {
      id: "s3",
      hotel: { name: "Pine View", city: "Almora" },

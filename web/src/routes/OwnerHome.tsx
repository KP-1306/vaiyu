...
import ObservabilityCard from "../components/ObservabilityCard"; // NEW (RBAC: owner/admin only)
import { useAuth } from "../lib/auth"; // RBAC context (role + profile)
...
export default function OwnerHome() {
  const { loading: authLoading, user, role, profile } = useAuth(); // role: 'owner' | 'staff' | 'admin' | 'guest'
  const [sp, setSp] = useSearchParams();
  const [slug, setSlug] = useState<string>(sp.get("slug") || (profile?.property_slug ?? "sunrise"));
  ...
  // Not signed in or wrong role
  if (!user || !["owner", "admin"].includes(role || "guest")) {
    return <Denied />;
  }

  // Signed in but has no property bound yet
  const hasProperty = Boolean(profile?.property_slug || slug);
  if (!hasProperty) {
    return <NoProperty />;
  }
          ...
          <div className="md:col-span-2 space-y-4">
            ...
            <section className="grid md:grid-cols-2 lg:grid-cols-3 gap-3" aria-label="Quick links">
              ...
              <Tile
                ...
                cta="View events"
              />
              <Tile title="Housekeeping" text="Live tickets with SSE (no refresh)." to={`/hk?slug=${encodeURIComponent(slug)}`} emoji="🧽" cta="Open HK" />
              <Tile title="Front Desk" text="Requests, SLAs, routing." to={`/desk?slug=${encodeURIComponent(slug)}`} emoji="🛎️" cta="Open desk" />
              <Tile
                title="Services (SLA)"
...
/* ---- RBAC Empty/Deny blocks (inline; feel free to move to components/EmptyStates.tsx) ---- */
function Denied() {
  return (
  ...
  return (
    <main className="max-w-xl mx-auto p-8 text-center space-y-4">
      ...
      <div className="flex gap-2 justify-center">
        ...
        <a className="btn btn-light" href="/owner/register">
          Register property
        </a>
      </div>
    </main>

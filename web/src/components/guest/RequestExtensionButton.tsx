...
export default function RequestExtensionButton({
  ...
  async function submit() {
    ...
    try {
      ...
      setErr(e instanceof Error ? e.message : t("stayDetails:extension.errSubmit"));
    } finally {
      setSubmitting(false);
    }
  }
  ...
  async function cancelPending() {
    ...
    try {
      ...
      setErr(e instanceof Error ? e.message : t("stayDetails:extension.errCancel"));
    } finally {
      setCancelling(false);
    }
  }
  ...
  const badge = latest && (
    ...
    >
      ...
      {/* status is an enum value (used in logic); translate only its display */}
      {t("stayDetails:extension.badge", { status: t(`stayDetails:extension.status.${latest.status}`), date: fmtDate(latest.requested_checkout_at) })}
    </div>
  );
    ...
    <>
      <div className={"flex flex-col items-start gap-2 " + (className ?? "")}>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {latest?.status === "pending" && (
            ...
            >
              {cancelling ? t("stayDetails:extension.cancelling") : t("stayDetails:extension.cancelRequest")}
            </button>
          )}
        </div>
        ...
        {latest?.status !== "pending" && (
          ...
          >
            ...
            {t("stayDetails:extension.requestExtension")}
          </button>
        )}
      </div>
      ...
      {open && createPortal(
        ...
        >
          ...
          >
            <h2 className="text-lg font-bold text-white mb-1">{t("stayDetails:extension.extendTitle")}</h2>
            <p className="text-xs text-slate-400 mb-5">
              {t("stayDetails:extension.currentCheckout")} <span className="text-slate-200 font-semibold">{fmtDate(currentCheckoutAt)}</span>.
              {" "}{t("stayDetails:extension.pickNewDate")}
            </p>

            <div className="space-y-4">
              <div className="space-y-2">
                <label className="text-[11px] font-bold uppercase tracking-wider text-slate-400">
                  {t("stayDetails:extension.newDate")}
                </label>
                <input
                  type="date"
                  value={date}
              ...
              <div className="space-y-2">
                <label className="text-[11px] font-bold uppercase tracking-wider text-slate-400">
                  {t("stayDetails:extension.reason")}
                </label>
                <textarea
                  value={note}
                  onChange={(e) => setNote(e.target.value)}
                  rows={3}
                  placeholder={t("stayDetails:extension.reasonPlaceholder")}
              ...
              {success && (
                <p className="text-sm text-emerald-300 flex items-center gap-2">
                  <CheckCircle2 className="w-4 h-4" /> {t("stayDetails:extension.submitted")}
                </p>
              )}
            </div>
            ...
            <div className="mt-6 flex justify-end gap-2">
              ...
              >
                ...
                {submitting ? t("stayDetails:extension.sending") : t("stayDetails:extension.submit")}
              </button>
            </div>
          </div>
        </div>,

-- 1. (Skipped) Priority column already exists with constraint ('LOW','NORMAL','HIGH','URGENT')

-- 2. Update RPC to accept priority and normalize to UPPERCASE
-- Drop ALL previous versions to ensure clean replacement (including old overloads)
DROP FUNCTION IF EXISTS create_service_request(UUID, UUID, UUID, UUID, TEXT, TEXT, UUID); -- 7 args (Oldest)
DROP FUNCTION IF EXISTS create_service_request(UUID, UUID, UUID, UUID, TEXT, TEXT, TEXT, UUID); -- 8 args (Old simplified)
DROP FUNCTION IF EXISTS create_service_request(UUID, UUID, UUID, UUID, TEXT, TEXT, UUID, UUID, JSONB); -- 9 args (Current)
DROP FUNCTION IF EXISTS create_service_request(UUID, UUID, UUID, UUID, TEXT, TEXT, UUID, UUID, JSONB, TEXT); -- 10 args (Self - just in case)

...
type Order = {
  id: string;
  status: string; // 'Placed' | 'Preparing' | 'Ready' | 'Delivered' (demo)
  created_at: string;
  items?: any[];
  room?: string;
  booking?: string;
...
export default function Desk() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setTickets((t?.items || []) as Ticket[]);
      setOrders((o?.items || []) as Order[]);
    } catch (e: any) {
    ...
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: 24, display: 'grid', gap: 16 }}>
      ...
      <section style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        ...
        <div className="card">
          ...
          {!tickets.length && <div style={{ marginTop: 8, color: 'var(--muted)' }}>No open HK requests.</div>}
        ...
        <div className="card">
          ...
          {!orders.length && <div style={{ marginTop: 8, color: 'var(--muted)' }}>No active orders.</div>}
          <div style={{ display: 'grid', gap: 8, marginTop: 10 }}>
            {orders.map((o) => (

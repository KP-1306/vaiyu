-- Lead events: snapshot actor display names in ALL state-mutating RPCs.
--
-- Day 3 reviewer flagged this for CLAIMED/CLAIM_RELEASED; we baked it in.
-- Day 4 added it to CONVERTED_TO_BOOKING. Day 9 timeline UI surfaces every
-- event type to operators — without snapshot names, the timeline reads
-- "user a1b2... changed status" which is operator-hostile.
--
-- This migration extends the same pattern consistently across the remaining
-- 7 RPCs. Additive change — does NOT bump event_schema_version (per protocol
-- in 20260525000008 comment). Old events without names render as "unknown"
-- in the timeline.

-- ─── create_lead — adds by_user_name to CREATED ────────────────────────────
...
BEGIN
  ...
  v_actor_name := public._user_display_name(auth.uid());
  ...
  IF p_notes IS NOT NULL AND btrim(p_notes) <> '' THEN
    ...
    VALUES (
      ...
      jsonb_build_object('text', p_notes, 'by_user_name', v_actor_name),
      auth.uid()
    );
  END IF;
...
-- ─── transition_lead_status — adds by_user_name to STATUS_CHANGED + REOPENED + CONVERTED_TO_BOOKING ─
...
BEGIN
  ...
  IF v_lead.status = 'LOST' AND p_to_status = 'NEW' THEN
    ...
    VALUES (p_lead_id, v_lead.hotel_id, 'REOPENED',
            jsonb_build_object(
              'previous_reason', v_lead.status_reason,
              'by_user_name', v_actor_name
            ), auth.uid());
  END IF;
...
-- ─── assign_lead — adds by_user_name + to_user_name + prev_user_name + from_user_name ──
...
BEGIN
  ...
  IF p_user_id IS NULL THEN
    ...
    VALUES (p_lead_id, v_lead.hotel_id, 'UNASSIGNED',
            jsonb_build_object(
              'from_user', v_lead.assigned_to,
              'from_user_name', public._user_display_name(v_lead.assigned_to),
              'by_user', auth.uid(),
              'by_user_name', v_actor_name
            ),
            auth.uid());
  ELSE
    ...
    VALUES (p_lead_id, v_lead.hotel_id, 'ASSIGNED',
            jsonb_build_object(
              'to_user', p_user_id,
              'to_user_name', public._user_display_name(p_user_id),
              'prev_user', v_lead.assigned_to,
              'prev_user_name', public._user_display_name(v_lead.assigned_to),
              'by_user', auth.uid(),
              'by_user_name', v_actor_name
            ),
            auth.uid());
...
-- ─── soft_delete_lead — adds by_user_name to SOFT_DELETED ─────────────────
...
BEGIN
  SELECT * INTO v_lead FROM public.leads WHERE id = p_lead_id FOR UPDATE;
  IF v_lead.id IS NULL THEN RAISE EXCEPTION 'LEAD_NOT_FOUND'; END IF;
  IF v_lead.deleted_at IS NOT NULL THEN RETURN; END IF;
...
-- ─── update_lead_contact — adds by_user_name to CONTACT_UPDATED ───────────
...
-- ─── update_lead_basics — adds by_user_name to BASICS_UPDATED ─────────────
...
-- ─── add_lead_note — adds by_user_name to NOTE_ADDED ──────────────────────
...
COMMENT ON FUNCTION public.create_lead IS 'v2: now snapshots by_user_name into CREATED + NOTE_ADDED event payloads (Day 9).';
COMMENT ON FUNCTION public.transition_lead_status IS 'v2: now snapshots by_user_name into STATUS_CHANGED + REOPENED + CONVERTED_TO_BOOKING (Day 9).';
COMMENT ON FUNCTION public.assign_lead IS 'v2: now snapshots by_user_name + to_user_name + prev_user_name + from_user_name into ASSIGNED + UNASSIGNED (Day 9).';
COMMENT ON FUNCTION public.soft_delete_lead IS 'v2: now snapshots by_user_name into SOFT_DELETED (Day 9).';
COMMENT ON FUNCTION public.update_lead_contact IS 'v2: now snapshots by_user_name into CONTACT_UPDATED (Day 9).';
COMMENT ON FUNCTION public.update_lead_basics IS 'v2: now snapshots by_user_name into BASICS_UPDATED (Day 9).';
COMMENT ON FUNCTION public.add_lead_note IS 'v2: now snapshots by_user_name into NOTE_ADDED (Day 9).';

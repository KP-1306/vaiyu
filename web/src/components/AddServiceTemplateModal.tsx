...
export default function AddServiceTemplateModal({
    ...
    const filteredTemplates = templates
        .filter(tpl => !existingServiceKeys.includes(tpl.code))
        .filter(tpl => tpl.label.toLowerCase().includes(searchTerm.toLowerCase()))
        .sort((a, b) => {
        ...
        <div className="vaiyu-owner fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div className="relative bg-[#1a1a1a] border border-gray-800 rounded-2xl shadow-2xl w-full max-w-3xl p-6 z-10 transition-all duration-300">
                ...
                <div className="flex items-center justify-between mb-6">
                    <div>
                        <h2 className="text-xl font-semibold text-white">
                            {t("modals.addServiceTemplate.title", "Add Services from Templates")}
                        </h2>
                        <p className="text-sm text-gray-400 mt-1">
                            {t("modals.addServiceTemplate.subtitle", "Select services to add from the available templates. Only services that haven't been added yet are shown.")}
                        </p>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <div className="space-y-4">
                    ...
                    <div className="flex gap-4 mb-4">
                        <div className="flex-1 relative">
                            ...
                            <input
                                type="text"
                                placeholder={t("modals.addServiceTemplate.searchPlaceholder", "Search service templates...")}
                                value={searchTerm}
                        ...
                        {!initialDepartmentId && (
                            <div className="flex items-center gap-2">
                                <span className="text-sm text-gray-400">{t("modals.addServiceTemplate.departmentLabel", "Department:")}</span>
                                <select
                        ...
                        <div className="flex items-center gap-2">
                            <span className="text-sm text-gray-400">{t("modals.addServiceTemplate.sortBy", "Sort by:")}</span>
                            <select
                                value={sortBy}
                            ...
                            >
                                <option value="name">{t("modals.addServiceTemplate.sortName", "SERVICE NAME")}</option>
                    ...
                    <div className="border border-gray-800 rounded-lg overflow-hidden bg-[#121212] h-[300px] overflow-y-auto">
                        <div className="grid grid-cols-[auto_1fr_150px] gap-4 p-3 bg-[#1e1e1e] border-b border-gray-800 text-xs font-semibold text-gray-400 uppercase tracking-wider">
                            ...
                            <div>{t("modals.serviceName", "Service Name")}</div>
                            <div>{t("modals.addServiceTemplate.colDefaultSla", "Default SLA")}</div>
                        </div>

                        {loadingTemplates ? (
                            <div className="flex items-center justify-center h-full text-gray-500">{t("modals.addServiceTemplate.loading", "Loading templates...")}</div>
                        ) : filteredTemplates.length === 0 ? (
                            <div className="flex flex-col items-center justify-center h-full text-gray-500 p-8 text-center">
                                <p>{t("modals.addServiceTemplate.noNew", "No new templates available for this department.")}</p>
                            </div>
                        ) : (
                            filteredTemplates.map(template => (
                                ...
                                >
                                    ...
                                    <div className="text-gray-400 text-sm">{t("modals.addServiceTemplate.minSuffix", "{{min}} min", { min: template.default_sla_minutes })}</div>
                                </div>
                            ))
                        )}
                    </div>
                    ...
                    <div className="flex justify-between items-center text-sm text-gray-500 mt-2">
                        <div>{t("modals.addServiceTemplate.viewing", "Viewing {{count}} templates", { count: filteredTemplates.length })}</div>
                    </div>
                </div>
                ...
                <div className="mt-8 pt-4 border-t border-gray-800 flex justify-end gap-3">
                    ...
                    >
                        {t("modals.addServiceTemplate.addSelected", "Add Selected Services")}{selectedTemplateIds.size > 0 ? ` (${selectedTemplateIds.size})` : ""} &gt;
                    </button>
                </div>
            </div>
        </div>

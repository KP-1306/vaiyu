-- ============================================================
-- VAiyu – Server-side discount cap
-- ============================================================
-- Audit gap closed: previously the only client-side check warned at
-- 20% but didn't hard-stop. A buggy client (or direct RPC call) could
-- give a 100% discount (free room). Now the server enforces a per-hotel
-- ceiling.
--
-- Design:
--   • New column pricing_settings.max_discount_pct (1..100, NULL = no cap)
--   • create_walkin_v2 fetches it once per call; if any room's
--     discount % exceeds the cap, the whole walk-in is rejected with a
--     clear error naming the offending percentage.
--   • Backward compatible: NULL means "no cap" — existing hotels keep
--     working until the operator sets a value.
-- ============================================================

ALTER TABLE public.pricing_settings
  ADD COLUMN IF NOT EXISTS max_discount_pct INT NULL
    CHECK (max_discount_pct IS NULL OR (max_discount_pct >= 1 AND max_discount_pct <= 100));

COMMENT ON COLUMN public.pricing_settings.max_discount_pct IS
  'Hard cap on per-room discount percentage at walk-in. NULL = no cap (engine warns at 20% client-side). Recommended: 50 for first deploy, tighten over time.';


-- ─── Replace create_walkin_v2 with cap-aware version ────────
-- Body is identical to the previous migration (20260425000001) plus a
-- small block at the top that loads max_discount_pct and a per-room
...
DECLARE
  ...
  v_max_discount_pct INT;       -- NEW: per-hotel cap
  v_discount_pct NUMERIC(5,2);  -- NEW: per-room calculated %
BEGIN
  ...
  IF v_any_discount THEN
    ...
    -- NEW: load the per-hotel discount cap once (per-loop repeat is wasteful).
  ...
  LOOP
    ...
    -- NEW: per-hotel discount-cap enforcement.

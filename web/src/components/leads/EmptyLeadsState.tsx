...
export function EmptyLeadsState({ onCreateClick }: Props) {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center">
      ...
      <h3 className="text-lg font-semibold text-white mb-1">{t('empty.title', 'No leads yet')}</h3>
      <p className="text-sm text-white/60 max-w-xs mb-6">
        {t('empty.body', 'Every enquiry you capture here becomes a follow-up. Start with the next call, walk-in, or referral.')}
      </p>
      <button
        type="button"
      ...
      >
        ...
        {t('empty.cta', 'Capture your first lead')}
      </button>
    </div>
  );
}

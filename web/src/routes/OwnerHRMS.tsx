    ...
    (async () => {
      ...
      try {
        ...
        const ids = Array.from(
          new Set((att || []).map((r: any) => r.user_id).filter(Boolean))
        );
        if (ids.length) {
                  ...
                  return (
                    <tr key={r.id} className="border-t">
                      ...
                      <td className="py-1 pr-3">
                        {profiles[r.user_id]?.full_name || r.user_id}
                      </td>
                      <td className="py-1 pr-3">
    ...
    (async () => {
      ...
      try {
        ...
        let query = supabase
          .from("leaves")
          .select(
            "id, user_id, start_date, end_date, type, status, created_at"
          )
          .eq("hotel_id", hotelId)
...
function StaffPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (role !== "all") {
      out = out.filter(
        (r: any) => (r.role || "").toLowerCase() === role.toLowerCase()
      );
    }
    if (q.trim()) {
  ...
  const roles = Array.from(
    new Set(rows.map((r: any) => r.role).filter(Boolean))
  );
...
export default function OwnerHRMS() {
  const [sp] = useSearchParams();
  const tab = sp.get("tab") || "attendance";

  let body: ReactNode;
  if (tab === "leaves") body = <LeavesPage />;
  else if (tab === "staff") body = <StaffPage />;
  else body = <AttendancePage />;

  return <HRMSLayout>{body}</HRMSLayout>;
}

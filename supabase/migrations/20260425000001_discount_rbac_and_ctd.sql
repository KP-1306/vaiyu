-- ============================================================
-- VAiyu Pricing – Discount RBAC + Checkout CTD enforcement
-- ============================================================
-- 1. Server-side enforcement: only OWNER/ADMIN/MANAGER/FINANCE_MANAGER
--    can post a discount via create_walkin_v2. SECURITY DEFINER means
--    the RPC bypasses RLS, so we need an explicit role check inside.
-- 2. CTD enforcement: a small helper plus a guard in checkout RPC(s) so
--    closed-to-departure dates can't be checked-out from.
-- ============================================================

-- ─── 1. Discount RBAC inside create_walkin_v2 ──────────────
-- We re-emit the function with the same body but a guard at the top of
-- the per-room loop. Function signature unchanged.
...
BEGIN
  ...
  -- Pre-scan for any positive discount in the payload. If found, the actor
  -- must be a finance-manager-or-above for THIS hotel; otherwise reject the
  -- whole request. This is a server-side guarantee — the UI also hides the
  -- field, but RLS + this check are the actual enforcement.
  ...
  IF v_any_discount THEN
    -- vaiyu_is_hotel_finance_manager() reads auth.uid() internally — this
    -- works whether the RPC is invoked by an authenticated user or by an
    -- explicit p_actor_id (we pass actor_id through `auth.uid()` from the
    -- client, but the RPC-side check is what enforces).
...
-- ─── 2. CTD helper for use by checkout flows ───────────────
-- A tiny helper that returns TRUE if any rate_restriction row marks a CTD
-- on the given (hotel, room_type?, date). NULL room_type_id matches
-- property-wide blocks. Frontend calls this to surface the block before
-- attempting the checkout RPC; checkout RPCs (added below) call it as
-- the final guard.

CREATE OR REPLACE FUNCTION public.is_closed_to_departure(
  p_hotel_id UUID,
  p_room_type_id UUID,
  p_date DATE
) RETURNS BOOLEAN
  ...
  SELECT EXISTS (
    SELECT 1 FROM public.rate_restrictions r
    WHERE r.hotel_id = p_hotel_id
      AND r.date     = p_date
      AND r.closed_to_departure = TRUE
      AND (r.room_type_id IS NULL OR r.room_type_id = p_room_type_id)
  );
$$;

GRANT EXECUTE ON FUNCTION public.is_closed_to_departure(UUID, UUID, DATE)
  TO authenticated, anon, service_role;

COMMENT ON FUNCTION public.is_closed_to_departure(UUID, UUID, DATE) IS
  'TRUE when the date is flagged closed-to-departure for this room type (or property-wide). Used by checkout flows to block guest departure on stop-departure dates.';

    ...
    >
        ...
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-1.5">
              <h3 className="text-sm font-semibold text-slate-100">Seasonal Demand Calendar</h3>
            ...
            <p className="text-[11px] text-slate-400 mt-0.5">
              Planning guide for regional travel seasons. No forecasts, no auto-campaigns.
            </p>
          </div>
        </div>
      ...
      {summary.topWindow ? (
        <div className="mt-3 rounded-lg border border-slate-800 bg-[#0B0E14] px-2.5 py-2">
          ...
          <div className="mt-0.5 flex items-center gap-1.5 text-[11px] text-slate-400">
            <span>{formatDaysUntil(summary.topWindow.days_to_start, summary.topWindow.computed_urgency)}</span>
            <span className="text-slate-600">·</span>
            <span className="font-semibold text-slate-300">{SEASONAL_URGENCY_LABEL[summary.topWindow.computed_urgency]}</span>
            <span className="text-slate-600">·</span>
            <span>{summary.topWindow.checklist_done}/{summary.topWindow.checklist_total} prep done</span>
          </div>
        </div>
      ) : (
        <div className="mt-3 text-[12px] text-slate-500">
          {listQ.isLoading
            ...
            : 'All planning windows are dismissed or hidden.'}
        </div>
      )}

      {/* Urgency counters */}
      <div className="mt-3 grid grid-cols-3 gap-2">
        <Metric icon={<Flame className="h-3 w-3" />} label="Now"     value={nowCount}   tone="rose" />
        <Metric icon={<Clock className="h-3 w-3" />} label="Prepare" value={prepCount}  tone="amber" />
        <Metric icon={<Eye className="h-3 w-3" />}   label="Watch"   value={watchCount} tone="sky" />
      </div>
      ...
      {readyCount > 0 && (
        <div className="mt-3 flex items-center gap-1.5 rounded-lg border border-slate-800 bg-[#0B0E14] px-2.5 py-1.5 text-[11px] text-slate-300">
          ...
          <span>{readyCount} marked READY by manager</span>
        </div>
      )}

      <p className="mt-3 text-[10px] text-slate-500">
        Deterministic planning windows. No demand prediction, no campaigns, no AI.
      </p>
    </Link>
  );
}

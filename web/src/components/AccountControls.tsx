...
export default function AccountControls({
  ...
  const switchToOwner = (slug?: string | null) => {
    ...
    try { localStorage.setItem("owner:slug", slug); } catch {}
    setOpen(false);
    navigate(`/owner/${slug}`);
  };
  ...
  const switchToStaff = (slug?: string | null) => {
    ...
    try { localStorage.setItem("staff:slug", slug); } catch {}
    setOpen(false);
    navigate("/staff");
  };
    ...
    <div ref={boxRef} className={`relative ${className}`}>
      ...
      {open && (
        ...
        >
          ...
          {/* Role switcher */}
          <div className="px-3 py-2">
            <div className="text-xs font-semibold text-gray-700 mb-1">Switch role</div>
            ...
            {loadingRoles && (
              <div className="text-xs text-gray-500 px-1 py-1.5">Loading memberships…</div>
            )}

            {!loadingRoles && owners.map((m) => (
              <MenuButton
                key={`own-${m.hotel_id}`}
            ...
            {!loadingRoles && memberships.some((m) => m.role === "manager") && owners

-- Visibility Score v3 — add Google Business Checklist signal
--
-- Adds one new TRUST_REPUTATION signal `gbp_checklist_ready` (AUTO_DERIVED)
-- that reads from the GBP Checklist's overall readiness via
-- `_gbp_signal_for_visibility(p_hotel_id)`. Bumps formula version 2 → 3.
--
...
--   ota_listing_ready       4 → 4  (unchanged)
--   gbp_checklist_ready     — → 4  (+4 NEW)
--   ----------------------------------------
...
--   • Existing snapshots carry formula_version=1 or 2 and remain interpretable
--   • New snapshots taken after this migration carry formula_version=3
...
AS $$
  SELECT 3::int AS version,
         jsonb_build_object(
           ...
           'map_pin_set',              5,
           'phone_present',            4,
           -- TRUST_REPUTATION (25) — internal rebalance for gbp_checklist_ready
           'review_link_set',          3,   -- was 4
           'reviews_flowing',          6,   -- was 7
           'off_platform_response',    3,   -- was 4
           'trust_essentials_assets',  5,   -- was 6
           'ota_listing_ready',        4,   -- unchanged
           'gbp_checklist_ready',      4,   -- NEW
...
COMMENT ON FUNCTION public._visibility_weights() IS
  'Authoritative weights for the Visibility Score formula. v3: added gbp_checklist_ready (TRUST_REPUTATION, weight 4); rebalanced 4 TRUST_REPUTATION signals (-1 each) to keep category at 25 and total at 100. TS mirror in web/src/config/visibilityScore.ts + vitest parity test enforces no silent drift.';

-- ─── _compute_visibility_score() v3 — adds gbp_checklist_ready signal ──────
-- Full replacement (CREATE OR REPLACE). Body mirrors v2 with one new signal
-- block inserted in TRUST_REPUTATION between ota_listing_ready (10c) and
...
DECLARE
  ...
  v_gbp_ready     boolean := false;  -- v3: GBP Checklist signal
...
BEGIN
  ...
  -- v3 NEW: GBP Checklist signal — partial-deploy safe via BEGIN/EXCEPTION
  BEGIN
  ...
  ----------------------------------------------------------------------------
  -- CATEGORY: TRUST_REPUTATION (v3: rebalanced -1 each on 4 signals; +4 NEW gbp)
  ----------------------------------------------------------------------------

  -- 7. review_link_set — v3 weight 3 (was 4)
  ...
  -- 8. reviews_flowing — v3 weight 6 (was 7)
  ...
  -- 9. off_platform_response — v3 weight 3 (was 4)
  ...
  -- 10. trust_essentials_assets — v3 weight 5 (was 6)
  ...
  -- 10b. ota_listing_ready (AUTO_DERIVED from OTA Optimizer; v2 unchanged)
  ...
  -- 10c. NEW v3: gbp_checklist_ready (AUTO_DERIVED from GBP Checklist module)
...
COMMENT ON FUNCTION public._compute_visibility_score(uuid) IS
  'Deterministic scoring of one hotel''s visibility readiness. v3: 21 signals (added gbp_checklist_ready in TRUST_REPUTATION). Loops through all signals, applies per-signal evaluation rules, accumulates contributions with min-sample carve-outs and 90-day manager-verification expiry, and returns a complete jsonb breakdown.';

-- ─── End of Visibility Score v3 migration ──────────────────────────────────

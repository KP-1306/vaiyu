    ...
    return (
      <main className="min-h-screen grid place-items-center bg-slate-50">
        <div className="max-w-md text-center space-y-3 px-4">
          <h1 className="text-xl font-semibold text-slate-800">Package not available</h1>
          <p className="text-sm text-slate-500">
            This package may have been paused or is no longer published. Please contact the property directly.
          </p>
          {hotelSlug && (
            <Link
    ...
    <main className="min-h-screen bg-slate-50 text-slate-900">
      ...
      {isPreview && (
        <div className="bg-amber-100 border-b border-amber-200 text-amber-900 px-4 py-2 text-xs sm:text-sm text-center">
          ...
          Preview mode — analytics not recorded. Approve and publish to make this page live.
        </div>
      )}
      <div className="mx-auto max-w-3xl px-4 sm:px-6 py-6 sm:py-10 space-y-5">
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-6 sm:p-8">
          ...
          <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3 text-sm">
            ...
            <div>
              ...
              <dd className="mt-0.5 text-slate-800">
                ...
                {pkg.max_party_adults && pkg.max_party_adults > pkg.min_party_adults
                  ...
                  : '+'}{' '}
                adult{pkg.min_party_adults === 1 && pkg.max_party_adults === 1 ? '' : 's'}
              </dd>
            </div>
            ...
            {(pkg.valid_from || pkg.valid_until) && (
              <div className="sm:col-span-2">
                <dt className="text-xs uppercase tracking-wide text-slate-500">Validity window</dt>
                <dd className="mt-0.5 text-slate-800">
                  {pkg.valid_from ? new Date(pkg.valid_from).toLocaleDateString('en-IN') : '—'}
                  {' to '}
                  {pkg.valid_until ? new Date(pkg.valid_until).toLocaleDateString('en-IN') : '—'}
                </dd>
              </div>
            )}
          </dl>
        ...
        <section className="rounded-3xl border border-emerald-200 bg-emerald-50/40 p-6 sm:p-8">
          ...
          <p className="mt-3 text-xs text-slate-600">
            Final rate confirmed by {hotel.name} after enquiry — depends on dates, party size, and inventory.
          </p>
          {enquireHref && (
            <Link
              to={enquireHref}
            ...
            >
              {pkg.enquiry_cta_label || 'Enquire now'}
              <ArrowRight className="h-3.5 w-3.5" aria-hidden />
            </Link>
          )}
        </section>
        ...
        <footer className="pt-2 pb-6 text-center text-xs text-slate-400">
          Powered by VAiyu · Hotel operations & guest journeys
        </footer>
      </div>
    </main>
  );

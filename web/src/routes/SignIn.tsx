...
export default function SignIn() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.warn("[SignIn] Error during initial session check:", err);
      } finally {
        if (!cancelled) {
  ...
  return (
    <div className="min-h-screen grid place-items-center bg-gray-50">
      <div className="w-full max-w-md rounded-2xl border bg-white p-6 shadow-sm min-h-[300px] flex flex-col justify-center">
        {checkingSession ? (
          <div className="flex flex-col items-center justify-center space-y-4">
              <div className="h-8 w-8 animate-spin rounded-full border-4 border-gray-200 border-t-sky-600"></div>
